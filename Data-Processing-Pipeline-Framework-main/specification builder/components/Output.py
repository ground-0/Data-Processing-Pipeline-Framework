...
class Output:
    ...
    def create_ui(self):
        ...
        self.area = dpg.add_collapsing_header(label="output" + str(self.num), parent=self.parent, before=self.before, indent=10)
        ...
        self.excel_file_output = dpg.add_input_text(label = "excel file path", filter_key="excel", source=self.excel_file, parent=self.filter)

        self.excel_tablelist = dpg.add_string_value( parent=self.registry)
        self.excel_tablelist_output = dpg.add_input_text(label = "excel table list", filter_key="excel", source=self.excel_tablelist, parent=self.filter)

        self.csv_file = dpg.add_string_value(parent=self.registry)
        self.csv_file_output = dpg.add_input_text(label = "csv file path", filter_key="csv", source=self.csv_file, parent=self.filter)

        self.csv_table_name = dpg.add_string_value(parent=self.registry)
        self.csv_table_name_output = dpg.add_input_text(label = "csv table name", filter_key="csv", source=self.csv_table_name, parent=self.filter)

        self.sql_host = dpg.add_string_value( parent=self.registry)
        self.sql_host_output = dpg.add_input_text(label = "sql host", filter_key="sql", source=self.sql_host, parent=self.filter)
        
        self.sql_db_name = dpg.add_string_value( parent=self.registry)
        self.sql_db_name_output = dpg.add_input_text(label = "sql database name", filter_key="sql", source=self.sql_db_name, parent=self.filter)

        self.sql_username = dpg.add_string_value( parent=self.registry)
        self.sql_username_output = dpg.add_input_text(label = "sql username", filter_key="sql", source=self.sql_username, parent=self.filter)

        self.sql_password = dpg.add_string_value( parent=self.registry)
        self.sql_password_output = dpg.add_input_text(label = "sql password", filter_key="sql", source=self.sql_password, parent=self.filter)

        self.sql_tablelist = dpg.add_string_value( parent=self.registry)
        self.sql_tablelist_output = dpg.add_input_text(label = "sql table list", filter_key="sql", source=self.sql_tablelist, parent=self.filter)

        dpg.add_button(label="remove output", parent=self.area, callback=self.delete_output_callback)
        ...
        dpg.bind_item_theme(self.area, output_theme)

    def create_xml(self):

        self.root = etree.Element(self.ns+"output", nsmap=self.nsmap)
        self.root.attrib["type"] = self.type

        if(self.type == "sql"):
            sql_host = etree.SubElement(self.root, self.ns+"host", nsmap=self.nsmap)
            sql_host.text = dpg.get_value(self.sql_host_output)
            sql_database = etree.SubElement(self.root, self.ns+"database", nsmap=self.nsmap)
            sql_database.text = dpg.get_value(self.sql_db_name_output)
            sql_username = etree.SubElement(self.root, self.ns+"username", nsmap=self.nsmap)
            sql_username.text = dpg.get_value(self.sql_username_output)
            sql_password = etree.SubElement(self.root, self.ns+"password", nsmap=self.nsmap)
            sql_password.text = dpg.get_value(self.sql_password_output)
            sql_tables = etree.SubElement(self.root, self.ns+"tables", nsmap=self.nsmap)
            sql_tables.text = dpg.get_value(self.sql_tablelist_output)

        elif(self.type == "excel"):
            excel_file = etree.SubElement(self.root, self.ns+"file", nsmap=self.nsmap)
            excel_file.text = dpg.get_value(self.excel_file_output)
            excel_tables = etree.SubElement(self.root, self.ns+"tables", nsmap=self.nsmap)
            excel_tables.text = dpg.get_value(self.excel_tablelist_output)

        elif(self.type == "csv"):
            csv_file = etree.SubElement(self.root, self.ns+"file", nsmap=self.nsmap)
            csv_file.text = dpg.get_value(self.csv_file_output)
            csv_table_name = etree.SubElement(self.root, self.ns+"table", nsmap=self.nsmap)
            csv_table_name.text = dpg.get_value(self.csv_table_name_output)

        return self.root

    def delete_output_callback(self, sender, appdata):
